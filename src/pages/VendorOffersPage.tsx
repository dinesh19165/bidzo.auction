...
export function VendorOffersPage() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload: OfferRequest = {
      ...
      ...(draft.discountType === 'PERCENTAGE' ? { maximumDiscountAmount: draft.maximumDiscountAmount === null || draft.maximumDiscountAmount === undefined || draft.maximumDiscountAmount === '' as never ? null : Number(draft.maximumDiscountAmount) } : {}),
          ...
          {editingId !== null ? (
            <Card className="p-5">
              ...
              <form onSubmit={submit} className="grid gap-4 md:grid-cols-2">
                ...
                <label><span className="mb-1.5 block text-sm text-slate-300">Discount type</span><select value={draft.discountType} onChange={(event) => { const discountType = event.target.value as DiscountType; updateDraft('discountType', discountType); if (discountType === 'FIXED_AMOUNT') updateDraft('maximumDiscountAmount', null); }} className="h-10 w-full rounded-lg border border-white/10 bg-slate-950/60 px-3 text-sm text-white"><option value="PERCENTAGE">Percentage</option><option value="FIXED_AMOUNT">Fixed amount</option></select></label>
                ...
                {draft.discountType === 'PERCENTAGE' ? <label><span className="mb-1.5 block text-sm text-slate-300">Maximum discount amount</span><input type="number" min="0" step="any" value={draft.maximumDiscountAmount ?? ''} onChange={(event) => updateDraft('maximumDiscountAmount', event.target.value === '' ? null : Number(event.target.value))} className="h-10 w-full rounded-lg border border-white/10 bg-slate-950/60 px-3 text-sm text-white" /></label> : null}
