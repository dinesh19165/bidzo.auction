...
export function OffersAdminPage() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (draft.discountType === 'PERCENTAGE' && draft.maximumDiscountAmount !== null && draft.maximumDiscountAmount !== undefined && (draft.maximumDiscountAmount === '' as never || !Number.isFinite(Number(draft.maximumDiscountAmount)) || Number(draft.maximumDiscountAmount) <= 0)) return setError('Maximum discount amount must be a positive amount when entered.');
  ...
  const toggle = async (offer: Offer) => { try { await updateOffer(offer.id, { name: offer.name, description: offer.description ?? '', offerType: offer.offerType, discountType: offer.discountType, discountValue: offer.discountValue, minimumOrderAmount: offer.minimumOrderAmount ?? null, ...(offer.discountType === 'PERCENTAGE' ? { maximumDiscountAmount: offer.maximumDiscountAmount ?? null } : {}), couponCode: offer.couponCode ?? null, usageLimit: offer.usageLimit ?? null, usageLimitPerCustomer: offer.usageLimitPerCustomer ?? null, startAt: offer.startAt ?? null, endAt: offer.endAt ?? null, priority: offer.priority, active: !offer.active, productIds: offer.productIds ?? [], categoryIds: offer.categoryIds ?? [] }); setOffers((current) => current.map((item) => item.id === offer.id ? { ...item, active: !offer.active } : item)); showToast('Offer status updated', 'Offer activation was updated.', 'success'); } catch (reason) { showToast('Status update failed', reason instanceof Error ? reason.message : 'Unable to update offer.', 'warning'); } };
  ...
  return <AdminShell title="Enterprise admin" subtitle="Offers" breadcrumbs={[{ label: 'Admin' }, { label: 'Offers' }]} activePath="/admin/offers" actions={<PrimaryButton onClick={beginCreate} icon={<Plus className="h-4 w-4" />}>Create offer</PrimaryButton>}>
    <div className="space-y-5">
      {editingId !== null ? <Card className="p-5"><div className="mb-4 flex items-center justify-between"><div><p className="text-xs uppercase tracking-[0.22em] text-blue-300">{editingId === 'new' ? 'New offer' : 'Edit offer'}</p><h2 className="mt-1 text-xl font-semibold text-white">Offer configuration</h2></div><SecondaryButton type="button" onClick={cancel}>Cancel</SecondaryButton></div><form onSubmit={submit} className="grid gap-4 md:grid-cols-2">
        ...
        <label className="block"><span className="mb-1.5 block text-sm text-slate-300">Discount type</span><select value={draft.discountType} onChange={(event) => { const discountType = event.target.value as DiscountType; update('discountType', discountType); if (discountType === 'FIXED_AMOUNT') update('maximumDiscountAmount', null); }} className="h-10 w-full rounded-lg border border-white/10 bg-slate-950/60 px-3 text-sm text-white">{discountTypes.map((value) => <option key={value} value={value}>{label(value)}</option>)}</select></label>
        {numberField('discountValue', 'Discount value')}{numberField('minimumOrderAmount', 'Minimum order amount')}{draft.discountType === 'PERCENTAGE' ? numberField('maximumDiscountAmount', 'Maximum discount amount') : null}{numberField('usageLimit', 'Usage limit')}{numberField('usageLimitPerCustomer', 'Usage limit per customer')}{numberField('priority', 'Priority')}
