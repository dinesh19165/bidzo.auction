      ...
      {loading ? <p className="text-sm text-slate-400">Loading addresses...</p> : addresses.length === 0 ? <p className="text-sm text-slate-400">No saved addresses. Add a delivery address to continue.</p> : compact && selectedAddressId && !showSavedAddresses ? (
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {addresses.map((address) => {
            ...
            return <button type="button" key={address.id} onClick={() => onSelect(address)} className={`${compact ? 'border-b border-white/10 py-2.5 last:border-b-0' : 'rounded-2xl border p-4'} w-full text-left text-sm transition ${selected ? 'text-white' : 'text-slate-300 hover:text-white'}`}>
              <div className="flex flex-wrap items-center gap-2"><p className="font-semibold">{address.fullName}</p>{compact ? <span className="rounded-md bg-white/10 px-2 py-0.5 text-[11px] text-slate-300">{address.label || 'Address'}</span> : null}</div>
