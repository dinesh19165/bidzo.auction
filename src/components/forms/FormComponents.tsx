...
export function Input({ label, placeholder, value, onChange, type = 'text', icon }: InputProps) {
  return (
    <label className="block text-sm text-slate-300">
      ...
      <div className="flex min-h-[48px] items-center gap-2 rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-3 text-sm text-white shadow-inner shadow-slate-900">
        {icon}
        <input type={type} value={value} onChange={onChange} placeholder={placeholder} className="w-full min-h-[48px] bg-transparent outline-none" />
      </div>
    </label>
  );
}
...
export function Select({ label, options, value, onChange }: { label?: string; options: Array<{ label: string; value: string }>; value?: string; onChange?: (event: ChangeEvent<HTMLSelectElement>) => void }) {
  return (
    <label className="block text-sm text-slate-300">
      {label && <span className="mb-2 block text-sm font-medium text-slate-200">{label}</span>}
      <select value={value} onChange={onChange} className="w-full min-h-[48px] rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-3 text-sm text-white outline-none">
