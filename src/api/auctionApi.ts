...
export function getEffectiveAuctionStatus(
  ...
  if (Number.isFinite(startTimestamp) && now < startTimestamp) {
    return 'SCHEDULED';
  }
...
/**
 * Get product images for an auction's product.
 *
 * Product images are stored separately from auctions:
...
function mapAuction(
  ...
  return {
    ...
    description:
      auction.description ||
      `Auction listing for product ${
        auction.productId ?? auction.id
      }`,

    // Use uploaded product image.
    // Fall back to logo only when no product image exists.
    ...
    endsIn: formatRelativeCountdown(
      auction.startAt,
      auction.endAt,
      auction.status
    ),

    seller: auction.vendorId
  ...
  try {
    ...
    const highestBid =
      bidAmounts.length > 0
        ? Math.max(...bidAmounts)
        : Number(
            String(auction.startingPrice).replace(
              /[^0-9.-]/g,
              ''
            )
          ) || 0;
...
export async function getAuctionImages(
  auctionId: number
): Promise<AuctionImageResponse[]> {
  const response = await fetchJson<
    ApiResponse<AuctionImageResponse[]>
  >(
...
export async function getAuctionWinner(
  auctionId: number
): Promise<AuctionWinnerResponse> {
  const response = await fetchJson<
    ApiResponse<AuctionWinnerResponse>
  >(
    `/api/auctions/${auctionId}/winner`,
    {
      method: 'GET',
    },
    false
...
export async function getAuctionRegistrationStatus(
  auctionId: number
): Promise<AuctionRegistrationStatusResponse> {
  const response = await fetchJson<
    ApiResponse<AuctionRegistrationStatusResponse>
  >(
    `/api/auctions/${auctionId}/registration`,
    {
      method: 'GET',
    }
  );
  ...
  if (!response?.data) {
    throw new Error(
      response?.message ||
        'Failed to load auction registration status'
    );
  }
...
export async function createAuctionRegistrationPayment(
  ...
  if (!response?.data) {
    throw new Error(
      response?.message ||
        'Failed to create auction registration payment'
    );
  }
...
export async function verifyAuctionRegistrationPayment(
  auctionId: number,
  request: RazorpayVerifyRequestDto
): Promise<PaymentResponseDto> {
  const response = await fetchJson<
    ApiResponse<PaymentResponseDto>
  >(
    `/api/auctions/${auctionId}/registration/payment/verify`,
    {
      method: 'POST',
  ...
  if (!response?.data) {
    throw new Error(
      response?.message ||
        'Failed to verify auction registration payment'
    );
  }
